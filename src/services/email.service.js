...
class EmailService {
  ...
  if (!this.isConfigured || !this.brevoApiKey) {
    ...
    // FIXED: Point to BACKEND for verification
    const backendUrl = process.env.BACKEND_URL || 'https://askyello-backend.onrender.com';
    console.log('Manual URL:', `${backendUrl}/api/v1/auth/verify-email?token=${verificationToken}`);
    return false;
  }

  try {
    // FIXED: Verification URL should point to BACKEND, not FRONTEND
    const backendUrl = process.env.BACKEND_URL || 'https://askyello-backend.onrender.com';
